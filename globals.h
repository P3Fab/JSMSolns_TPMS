...
//   const int MAXTIMINGS = 255;
   ...
   #define PROC_TYPE "Seeeduino Xiao"
   const int RXPin = D1;   //must be an ext interrupt pin
   const int CDPin = D0;  //if wired, define 'USE_HW_CD' above, otherwise CD pin status is received over SPI
   const int CC1101_CS = D2;  // Define the Chip Select pin
   const int AUDIBLE_ALARM_PIN = D7;
   const int DEBUGPIN = 16;  //use the TX LED pin
   const int LED_RX = 17;
   const int MAXBITS = 1000;
   const int MAXTIMINGS = 900;
   ...
   const int AUDIBLE_ALARM_PIN = 5;
   const int DEBUGPIN = 6;  //use the TX LED pin
   const int LED_RX = LED_BUILTIN;
   const int MAXBITS = 1000;
   const int MAXTIMINGS = 900;
