...
void DecodeTPMS()
{
 ...
 // id = ((unsigned long)RXBytes[1] & 0x0F) << 24 | (unsigned long)RXBytes[2] << 16 | (unsigned long)RXBytes[3] << 8 | (unsigned long)RXBytes[3];
...
bool ValidateTimings()
{
  ...
  int ret;
  //unsigned long DecodeStartTime = micros();
...
//           Serial.print(micros() - DecodeStartTime);
//           Serial.println("us");
        //decode the message...
        DecodeTPMS();
